...
static int dispatch(int argc, char **argv)
{
  ...
  for (i = 0; i < sizeof(g_commands) / sizeof(g_commands[0]); i++) {
    ...
    LOG_DEBUG("dispatch command=%s sub=%s argc=%d", g_commands[i].name,
              g_commands[i].sub ? g_commands[i].sub : "(none)", argc);
    if (strcmp(g_commands[i].name, "serve") != 0) {
      chippy_apply_dir_flag(argc, argv);
    }
    rc = g_commands[i].run(argc, argv);
    LOG_DEBUG("command %s finished rc=%d", g_commands[i].name, rc);
    if (rc < 0) {
      usage(argv[0]);
      return 1;
    }
...
{
  LOG_DEBUG("chippy starting argc=%d", argc);
  if (sodium_init() < 0) {
    fprintf(stderr, "sodium_init failed\n");
    return 1;
  }
  LOG_DEBUG("sodium_init ok");
