...
Mint authority secrets stay off-server. `config` lists `authorized_mint_key=<64 hex>` lines (multiple allowed). Mint txs must be signed by any listed key. Use `chippy mint-key add <addr>` to authorize another minter.
